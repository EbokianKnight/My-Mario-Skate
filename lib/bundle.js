...
/***/ function(module, exports, __webpack_require__) {
	...
	document.addEventListener("DOMContentLoaded", function () {
	  ...
	  var game = new Game(context, width, height, bloc, mobile);
	
	  var view = new GameView(game, ctx, bloc, mobile);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Game = function (context, width, height, size, mobile) {
	  DIMX = width;
	  DIMY = height;
	  SPACER = size;
	  this.mario = new Mario({ x: DIMX / 2, y: DIMY / 2 }, SPACER, mobile);
	...
	Game.prototype.createShells = function () {
	  ...
	  var shellSpeed = this.mobile ? 3 : 2;
	  var vel = Util.randomVec(shellSpeed);
	...
	// on the mobile, less shells will spawn due to difficulty.
	Game.prototype._numShells = function () {
	  if (this.mobile) {
	    return 3 + Math.floor(this.score.points / 20) - destroyedShells;
	  } else {
	    return 4 + Math.floor(this.score.points / 12) - destroyedShells;
	  }
	};
	...
	Game.prototype.checkCollisions = function () {
	  ...
	  for (i = 0; i < coins.length; i++) {
	    current = coins[i];
	    if (!current.collected && current.isCollidedWith(this.mario)) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	Shell.prototype.draw = function (ctx) {
	  ...
	  if (this.ghost) {
	    ...
	    ctx.drawImage(document.getElementById('green-reflect'), // sprite
	    0 + offset, // offset x
	    0, // offset y
	    40, // frameWidth
	    40, // frameHeight
...
/***/ function(module, exports, __webpack_require__) {
	...
	function Mario(position, size, mobile) {
	...
	Mario.prototype.power = function (impulse) {
	  ...
	  if (length > this.maxVelocity * this.maxVelocity) {
	...
	Mario.prototype.walk = function (dir, velMod) {
	...
	Mario.prototype.draw = function (ctx) {
	  ...
	  44, // frameWidth
	  65, // frameHeight
	  this.position.x - this.size * 0.7, // draw at x
	  this.position.y - this.size * 1.4, // draw at y
	  this.size * 1.4, // frameWidth
	  this.size * 2.08 // frameHeight
	  );
	  if (this.invincible) {
	    ctx.restore();
	  } else {
	    ctx.drawImage(document.getElementById('reflect'), // sprite
	    facing.dx + offset, // offset x
	    facing.dy, // offset y
	    44, // frameWidth
	    65, // frameHeight
	    this.position.x - this.size * 0.7, // draw at x
	    this.position.y + this.size * 0.4, // draw at y
	    this.size * 1.4, // frameWidth
	    this.size * 2.08 // frameHeight
	    );
	  }
...
/***/ function(module, exports) {
	...
	GameView.prototype.loadNewGameModal = function () {
	  ...
	  if (!this.mobile) {
	    this.fnID = this.startGame.bind(this);
	    this.button.addEventListener("click", this.fnID);
	  }
	};
	...
	GameView.prototype.startGame = function () {
	  ...
	  this.button.removeEventListener("click", this.fnID);
	...
	GameView.prototype.bindKeyHandlers = function () {
	  ...
	  this.orient = window.addEventListener('deviceorientation', function (event) {
	    ...
	    var moreVel = 4;
	    var mostVel = 6;
	    if (event.beta > minTilt && event.gamma > minTilt) {
	      //down right
	    ...
	    } else if (event.beta > minTilt && event.gamma < -minTilt) {
	      //down left
	      that.keysDown.up = false;
	    ...
	    } else if (event.gamma < -minTilt) {
	      // left
	      that.keysDown.up = false;
	      that.keysDown.right = false;
	      that.keysDown.down = false;
	      that.keysDown.left = true;
	      if (event.gamma < -moreTilt) {
	        this.velMod = moreVel;
	      } else if (event.gamma < -mostTilt) {
	        this.velMod = mostVel;
	      } else {
	        this.velMod = minVel;
	      }
	    ...
	    } else if (event.gamma > -minTilt && event.gamma < minTilt && event.beta > -minTilt && event.beta < minTilt) {
	      //middle-ish
	      that.keysDown.up = false;
	      that.keysDown.right = false;
	      that.keysDown.down = false;
